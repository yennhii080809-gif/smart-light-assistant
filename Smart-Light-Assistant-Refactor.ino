...
unsigned long configTimeoutMs = DEFAULT_TIMEOUT_SECONDS * 1000UL;
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        body { background: linear-gradient(135deg, #0f2027, #203a43, #2c5364); color: #fff; display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 20px; }
        .container { background: rgba(255, 255, 255, 0.06); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 16px; padding: 30px; width: 100%; max-width: 450px; box-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.37); text-align: center; }
        ...
        button { width: 100%; padding: 14px; background: linear-gradient(45deg, #0093e9, #80d0c7); border: none; border-radius: 8px; color: #fff; font-size: 18px; font-weight: bold; cursor: pointer; transition: 0.3s; box-shadow: 0 4px 15px rgba(0, 147, 233, 0.4); margin-top: 10px; }
    ...
    <script>
        window.onload = function() {
            fetch('/get-config').then(response => response.json()).then(data => {
                ...
                if(data.relayState) { statusBox.innerText = "ĐANG BẬT"; statusBox.className = "status on"; }
                else { statusBox.innerText = "ĐANG TẮT"; statusBox.className = "status off"; }
            });
        };
        document.getElementById('configForm').addEventListener('submit', function(e) {
            ...
            fetch(`/set-config?timeout=${timeout}&volume=${volume}`).then(response => response.text()).then(data => {
...
void handleGetConfig() {
    bool rState = relay.isOn();
    char jsonBuffer[128];
    snprintf(jsonBuffer, sizeof(jsonBuffer), "{\"timeout\":%lu,\"volume\":%d,\"relayState\":%s}", configTimeoutMs / 1000, currentVolumePercent, rState ? "true" : "false");
...
void handleSetConfig() {
    if (server.hasArg("timeout") && server.hasArg("volume")) {
        ...
        if (t >= MIN_TIMEOUT_SECONDS && t <= MAX_TIMEOUT_SECONDS && v >= MIN_VOLUME_PERCENT && v <= MAX_VOLUME_PERCENT) {
...
void setup() {
    ...
    if (!WiFi.softAP(apSSID, apPassword)) Serial.println(F("[WiFi] Không thể khởi tạo Access Point!"));
    else { Serial.print("[WiFi] AP Ready. IP Web: "); Serial.println(WiFi.softAPIP()); }
    ...
    delay(1000);
    if (!dfPlayer.begin()) Serial.println(F("[DFPLAYER] Không tìm thấy module hoặc thẻ nhớ!"));
    else { dfPlayer.setVolumePercent(currentVolumePercent); Serial.println(F("[DFPLAYER] Sẵn sàng đọc file từ thư mục mp3.")); }

    randomSeed(esp_random());

#if ESP_ARDUINO_VERSION_MAJOR >= 3
    esp_task_wdt_config_t wdt_config = { .timeout_ms = WDT_TIMEOUT_SECONDS * 1000, .idle_core_mask = (1 << 0), .trigger_panic = true };
